...
{empty} +
{empty} +
HEFWriteWord is used to write information, word values, to the HEFM data storage, so that it can be accessed later by a programmer on a Personal, or by the HEFMRead commands.
{empty} +
{empty} +
...
{empty} +
`data` is the data that is to be written to the HEFM data storage.&#160;&#160;&#160;A word value.&#160;&#160;&#160;This can be a word value or a word variable.
{empty} +
{empty} +
