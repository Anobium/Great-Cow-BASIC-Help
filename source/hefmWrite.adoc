...
{empty} +
{empty} +
The method will compute the row number and the offset in the row from `location` and Great Cow BASIC constant HEFLASH_ROWSIZE.&#160;&#160;&#160;For more information on the Great Cow BASIC constants that supprt the HEF operations see <<_hefmoverview, HEF Memory Overview>>
{empty} +
{empty} +
