...
{empty} +
{empty} +
HEFWrite is used to write information, byte values, to the HEFM data storage, so that it can be accessed later by a programmer on a Personal, or by the HEFMRead commands.
{empty} +
{empty} +
...
{empty} +
`data` is the data that is to be written to the HEFM data storage.&#160;&#160;&#160;A byte value.&#160;&#160;&#160;
