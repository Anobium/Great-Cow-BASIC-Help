...
Where a hard reset reverts ram back to and undefined /random state!&#160;&#160;
...
{empty} +
{empty} +
*How to change the compilers behaviour when the compiler states a capability is not available when I know it is ?*
...
{empty} +
{empty} +
*How do I create a minimal ASM source with no config and/or initsys?*
