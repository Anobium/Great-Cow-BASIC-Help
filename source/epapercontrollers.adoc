...
To make the operation of the library seamless - the library supports GLCD Transaction.&#160;&#160;
GLCD Transaction automatically manages the methods to update the e-paper via the buffer, where the buffer can be small.&#160;&#160;
...
Transaction simplies the operation by ensure the buffer is setup correctly, handles the GLCD appropiately, handles the sending of the buffer and then close out the update to the display.  

To use transaction use the followng methods.
...
When using GLCDtransation you must use `GLCD_Open_PageTransaction` then a series of GLCD commands and then `GLCD_Close_PageTransaction`.
...
|`GLCD_Close_PageTransaction`. Parameters may be passed where two parameters are the range of pages to be updated
