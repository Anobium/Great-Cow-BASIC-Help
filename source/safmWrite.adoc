...
{empty} +
{empty} +
SAFWrite is used to write information, byte values, to the HFEM data storage, so that it can be accessed later by a programmer on a Personal, or by the SAFRead commands.
{empty} +
{empty} +
