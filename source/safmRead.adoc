...
----
    'as a subroutine
    SAFMRead ( location, data )

    'as a function
    data = SAFMRead ( location )
----
...
{empty} +
{empty} +
SAFMRead is used to read information, byte values, from the HFEM data storage, so that it can be accessed for use in a user program.
{empty} +
{empty} +
