...
{empty} +
{empty} +
SAFRead is used to read information, byte values, from the HFEM data storage, so that it can be accessed for use in a user program.
{empty} +
{empty} +
...
{empty} +
{empty} +
This method reads data from the SAFM given the specific location the SAFM data storage.&#160;&#160;&#160;
...
This method appropriately handles the management of the SAFM data storage by calculating the correct SAFM page and reading the byte from the appropriate SAFM page.
{empty} +
{empty} +
*Example 2:*
----
...
{empty} +
For more information on the Great Cow BASIC constants that support the HEF operations see <<_safm_overview, SAF Memory Overview>>
...
----
    ...
    sub SAFM_Dump
        ...
        for index = 0 to SAF_WORDS - 1
          test=index%8
          test1=index%SAF_ROWSIZE_BYTES
          row=SAF_START_ADDR+index
          if test1 = 0 then
              HSerPrintCRLF
              hserprint "        0"
              hserprint "     1"
